...
def insertCommonDataSQL(db, cursor, testID, opts):
    ...
    :return: Tuple of the the id of the newly inserted item, and success of the db operation
    """
    sql = "INSERT INTO `CommonData` (`id_Experiments`,`protocol`,`num_packets`,`num_tail`,`packet_size`, `tail_wait` ) VALUES " \
          " ('%d','%s','%s','%s','%s', '%s');" % (
              testID, opts["trans_proto"], opts["num_packets"], opts["num_tail"], opts["data_length"], opts["tail_wait"])
    return insertToSQL(db, cursor, sql)

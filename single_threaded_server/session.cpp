...
namespace detection
{
    ...
    session::session(int tcp_sock, sockaddr_in client_in) : client(client_in)
    {
        tcp_fd   = tcp_sock;
        tcp_open = true;
    ...
    int session::setupUdpSocket(uint16_t port)
    {
    ...
    void session::processUdpPacket()
    {
        char recv_buff[1500] = {};
        uint16_t* id         = (uint16_t*)recv_buff;
        int n = recvfrom(udp_fd, recv_buff, sizeof(recv_buff), 0, reinterpret_cast<sockaddr*>(&client), &client_len);
    ...
    test_params session::getTcpParams()
    {
    ...
    void session::terminate_session(std::string msg)
    {
        std::cerr << msg << std::endl;
        exit(-1);
    }

    void session::closeUdp()
    {
        if(udp_open)
        {
            close(udp_fd);

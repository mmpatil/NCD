...
int recv_ready = 0;                      // bool for receiving SYN packets -- denotes if the program
...
int comp_det()
{
    ...
    /*taken from
     * http://stackoverflow.com/questions/17914550/getaddrinfo-error-success*/
    if(err != 0)
    {
        if(err == EAI_SYSTEM)
    ...
    int icmp_hdrincl = 1;
    if(setsockopt(icmp_fd, r, IP_HDRINCL, &icmp_hdrincl, sizeof(icmp_hdrincl)) == -1)
    {
    ...
    if(lflag && hflag)
    {
        ...
        if(tcp_bool == 1)
        {
            do
            {
                err = recvfrom(send_fd, buff, 1500, MSG_DONTWAIT, NULL, NULL);
            } while(err != -1);
        ...
        do
        {
            err = recvfrom(recv_fd, buff, 1500, MSG_DONTWAIT, NULL, NULL);
        } while(err != -1);
...
int mkipv6(void* buff, size_t size, u_int8_t proto)
{
    ...
    ip->ip6_ctlun.ip6_un2_vfc = proto;        //<-------------wrong, just want to get
    // rid of warning. ipv6 doesn't work.
...
int setup_tcp_packets()
{
    ...
    //!!! we're moving this ptr wrong somehow we get 0 for ports and other
    //! errors
...
void* send_udp()
{
    ...
    // tail wait is in milliseconds, so multiply by 10^6 to convert to
    // nanoseconds
...
void* recv4(void* t)
{
    ...
    if(tcp_bool == 1)
    {
        ...
        do
        {
            if((n = recvfrom(send_fd, packet_rcv, sizeof(packet_rcv), 0, (struct sockaddr*)&addr, &adrlen)) < 0)
            {
                perror("recvfrom failed");
            }

        } while((tcp->dest != htons(syn_port)) || (ip->ip_src.s_addr != destip.s_addr));
        *time = get_time() - td;
    ...
    else
    {
        ...
        for(;;)
        {
            ...
            if((n = recvfrom(recv_fd, packet_rcv, sizeof(packet_rcv), 0, (struct sockaddr*)&addr, &adrlen)) < 0)
            {
                if(errno == EINTR)
                    continue;
                perror("recvfrom failed");
...
void* recv6(void* t)
{
    ...
    for(;;)
    {
        ...
        if((n = recvfrom(recv_fd, packet_rcv, icmp_len, 0, (struct sockaddr*)&addr, &adrlen)) < 0)
        {
            if(errno == EINTR)
                continue;
